import asyncio
import os
import sys

from dotenv import load_dotenv
from fastapi import WebSocket
from loguru import logger

from pipecat.pipeline.pipeline import Pipeline
from pipecat.pipeline.runner import PipelineRunner
from pipecat.pipeline.task import PipelineParams, PipelineTask
from pipecat.frames.frames import InputTextRawFrame
from pipecat.serializers.twilio import TwilioFrameSerializer
from pipecat.services.google.gemini_live import GeminiLiveLLMService
from pipecat.services.google.gemini_live.llm import GeminiModalities
from pipecat.transports.websocket.fastapi import (
    FastAPIWebsocketParams,
    FastAPIWebsocketTransport,
)

from prompts import get_system_prompt

load_dotenv(override=True)

try:
    logger.remove(0)
except ValueError:
    pass
logger.add(sys.stderr, level="DEBUG")

google_api_key = os.getenv("GOOGLE_API_KEY")


async def run_bot(websocket_client: WebSocket, stream_sid: str, testing: bool, customer_name: str = ""):
    logger.info(f"Starting bot for stream_sid={stream_sid}")

    transport = FastAPIWebsocketTransport(
        websocket=websocket_client,
        params=FastAPIWebsocketParams(
            audio_in_enabled=True,
            audio_out_enabled=True,
            add_wav_header=False,
            vad_enabled=False,
            serializer=TwilioFrameSerializer(
                stream_sid,
                params=TwilioFrameSerializer.InputParams(auto_hang_up=False),
            ),
        ),
    )

    # Build system instruction with customer name context
    system_prompt = get_system_prompt()
    if customer_name:
        system_prompt += f"\n\nIMPORTANT: This is an outbound call. The customer's name is {customer_name}. Greet them by name and introduce yourself immediately."
    else:
        system_prompt += "\n\nA caller has just connected. Introduce yourself immediately without waiting for them to speak first."

    llm = GeminiLiveLLMService(
        api_key=google_api_key,
        system_instruction=system_prompt,
        settings=GeminiLiveLLMService.Settings(
            model="models/gemini-3.1-flash-live-preview",
            voice="Kore",
            modalities=GeminiModalities.AUDIO,
            temperature=0.6,
            max_tokens=200,
        ),
        inference_on_context_initialization=True,
    )

    pipeline = Pipeline(
        [
            transport.input(),
            llm,
            transport.output(),
        ]
    )

    task = PipelineTask(
        pipeline,
        params=PipelineParams(
            audio_in_sample_rate=8000,
            audio_out_sample_rate=8000,
            allow_interruptions=True,
            enable_metrics=True,
            enable_usage_metrics=True,
        ),
        idle_timeout_secs=300,
        cancel_on_idle_timeout=True,
    )

    @transport.event_handler("on_client_connected")
    async def on_client_connected(transport, client):
        logger.info(f"[{stream_sid}] Client connected, customer_name={customer_name or 'inbound'}")
        # Wait for Gemini session to be fully connected
        while not llm._session:
            await asyncio.sleep(0.1)
        logger.info(f"[{stream_sid}] Gemini session ready — triggering greeting")
        await task.queue_frames([InputTextRawFrame(text="Hello.")])

    @transport.event_handler("on_client_disconnected")
    async def on_client_disconnected(transport, client):
        logger.info(f"[{stream_sid}] Client disconnected")
        await task.cancel()

    @task.event_handler("on_pipeline_error")
    async def on_pipeline_error(task, error):
        logger.error(f"[{stream_sid}] Pipeline error: {error}")

    runner = PipelineRunner(handle_sigint=False, force_gc=True)

    await runner.run(task)
    logger.info(f"[{stream_sid}] Bot session ended")
